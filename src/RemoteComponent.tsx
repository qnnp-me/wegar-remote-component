...
const RemoteComponent = <T extends Record<string, unknown>>(componentProps: RemoteComponentProps<T>) => {
  const {
    url,
    css = false,
    name,
  ...
  const componentFunction = (typeof window[UUID as never] === 'function' ? window[UUID as never] : window[componentName as never]) as unknown as (FunctionComponent & {
    React?: typeof React
  }) | undefined
  if (componentLoading) {
...
declare interface RemoteComponentProps<T extends Record<string, unknown>> {
  /**
   * 组件文件地址
   */
  url: string
